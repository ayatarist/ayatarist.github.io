...
interface TabsProps {
  active: SectionKey
  setActive: Dispatch<SetStateAction<SectionKey>>
}

const Tabs: React.FC<TabsProps> = ({ active, setActive }) => (
  <div className="flex flex-wrap gap-3 mb-10 p-2 bg-white/70 backdrop-blur-sm rounded-2xl border border-orange-100">
    {(Object.keys(sectionConfig) as SectionKey[]).map((key) => {
      ...
      return (
        <button
          ...
          className={`px-8 py-4 rounded-xl font-bold transition-all duration-300 text-sm ${
            isActive
...
export default function ProfessionalTimeline() {
  ...
  return (
    <section
      id="timeline"
      className="relative w-full min-h-screen bg-white py-19 px-9"
    >
      <div
